...
export class ApiGeneralService extends ApiBaseService {
  ...
  public makeText(obj: MakeTextDto): Observable<string> {
    ...
    return this.http.post<string>(apiUrl, obj).pipe(catchError(ErrorHandler.handlerError));
  }

  public hasLinkedin(): Observable<boolean> {
    const apiUrl = `${this.baseUrl}/linkedin/can-pass`;
    return this.http.get<boolean>(apiUrl).pipe(catchError(ErrorHandler.handlerError));
  }

  public makePost(obj: MakePostDto): Observable<void> {
    const apiUrl = `${this.baseUrl}/linkedin/can-pass`;
    return this.http.post<void>(apiUrl, obj).pipe(catchError(ErrorHandler.handlerError));
  }
